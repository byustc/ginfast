...
// EntMember res_enterprise_member 模型结构体
...
// EntMemberList res_enterprise_member 列表
type EntMemberList []EntMember

// NewEntMember 创建res_enterprise_member实例
func NewEntMember() *EntMember {
	return &EntMember{}
}

// NewEntMemberList 创建res_enterprise_member列表实例
func NewEntMemberList() *EntMemberList {
	return &EntMemberList{}
}

// TableName 指定表名
func (EntMember) TableName() string {
	return "res_enterprise_member"
}

// GetByID 根据ID获取res_enterprise_member
func (m *EntMember) GetByID(c context.Context, id int64) error {
	return app.DB().WithContext(c).First(m, id).Error
}

// Create 创建res_enterprise_member记录
func (m *EntMember) Create(c context.Context) error {
	return app.DB().WithContext(c).Create(m).Error
}

// Update 更新res_enterprise_member记录
func (m *EntMember) Update(c context.Context) error {
	return app.DB().WithContext(c).Save(m).Error
}

// Delete 软删除res_enterprise_member记录
func (m *EntMember) Delete(c context.Context) error {
	return app.DB().WithContext(c).Delete(m).Error
}

// IsEmpty 检查模型是否为空
func (m *EntMember) IsEmpty() bool {
	return m == nil || m.Id == 0
}

// Find 查询res_enterprise_member列表
func (l *EntMemberList) Find(c context.Context, funcs ...func(*gorm.DB) *gorm.DB) error {
	return app.DB().WithContext(c).Model(&EntMember{}).Scopes(funcs...).Find(l).Error
}

// GetTotal 获取res_enterprise_member总数
func (l *EntMemberList) GetTotal(c context.Context, query ...func(*gorm.DB) *gorm.DB) (int64, error) {
	var count int64
	err := app.DB().WithContext(c).Model(&EntMember{}).Scopes(query...).Count(&count).Error
	return count, err
}