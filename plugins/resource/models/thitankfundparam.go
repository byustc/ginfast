...
// ThitankFundListRequest res_thinktank_fund列表请求参数
type ThitankFundListRequest struct {
	models.BasePaging
	models.Validator
	Id *int64 `form:"id"` // 主键
	...
	InvestIndustry *string `form:"investIndustry"` // 投资行业偏好
	...
	InvestmentIndustries *string `form:"investmentIndustries"` // 投资产业
	AvailableBalanceOfFund *string `form:"availableBalanceOfFund"` // 基金可投余额
	Tag *string `form:"tag"` // 数据标签
	UpdatedBy *int64 `form:"updatedBy"` // 修改人
}

// Validate 验证请求参数
func (r *ThitankFundListRequest) Validate(c *gin.Context) error {
	return r.Validator.Check(c, r)
}

// Handle 获取查询条件
func (r *ThitankFundListRequest) Handle() func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
        ...
        if r.InvestmentIndustries != nil {
            // 默认等于查询
            db = db.Where("investment_industries = ?", *r.InvestmentIndustries)
        }
        if r.AvailableBalanceOfFund != nil {
            // 默认等于查询
            db = db.Where("available_balance_of_fund = ?", *r.AvailableBalanceOfFund)
        }
        if r.Tag != nil {
            // 默认等于查询
...
// ThitankFundCreateRequest 创建res_thinktank_fund请求参数
type ThitankFundCreateRequest struct {
	models.Validator
	...
	ThinktankId int64 `form:"thinktankId"` // 新研机构-数据主键id
	InvestIndustry string `form:"investIndustry" validate:"required" message:"投资行业偏好不能为空"` // 投资行业偏好
	...
	IsBlacklist int `form:"isBlacklist"` // 是否负面清单: 0否 1是
	InvestmentIndustries string `form:"investmentIndustries" validate:"required" message:"投资产业不能为空"` // 投资产业
	AvailableBalanceOfFund string `form:"availableBalanceOfFund" validate:"required" message:"基金可投余额不能为空"` // 基金可投余额
	Tag string `form:"tag" validate:"required" message:"数据标签不能为空"` // 数据标签
	UpdatedBy int64 `form:"updatedBy"` // 修改人
}

// Validate 验证请求参数
func (r *ThitankFundCreateRequest) Validate(c *gin.Context) error {
	return r.Validator.Check(c, r)
}

// ThitankFundUpdateRequest 更新res_thinktank_fund请求参数
type ThitankFundUpdateRequest struct {
	models.Validator
...
// Validate 验证请求参数
func (r *ThitankFundUpdateRequest) Validate(c *gin.Context) error {
	return r.Validator.Check(c, r)
}

// ThitankFundDeleteRequest 删除res_thinktank_fund请求参数
type ThitankFundDeleteRequest struct {
	models.Validator
	Id int64 `form:"id" validate:"required" message:"主键不能为空"` // 主键
}

// Validate 验证请求参数
func (r *ThitankFundDeleteRequest) Validate(c *gin.Context) error {
	return r.Validator.Check(c, r)
}

// ThitankFundGetByIDRequest 根据ID获取res_thinktank_fund请求参数
type ThitankFundGetByIDRequest struct {
	models.Validator
	Id int64 `uri:"id" validate:"required" message:"主键不能为空"` // 主键
}

// Validate 验证请求参数
func (r *ThitankFundGetByIDRequest) Validate(c *gin.Context) error {
	return r.Validator.Check(c, r)
}