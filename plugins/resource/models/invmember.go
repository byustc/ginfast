...
// InvMember res_invest_member 模型结构体
...
// InvMemberList res_invest_member 列表
type InvMemberList []InvMember

// NewInvMember 创建res_invest_member实例
func NewInvMember() *InvMember {
	return &InvMember{}
}

// NewInvMemberList 创建res_invest_member列表实例
func NewInvMemberList() *InvMemberList {
	return &InvMemberList{}
}

// TableName 指定表名
func (InvMember) TableName() string {
	return "res_invest_member"
}

// GetByID 根据ID获取res_invest_member
func (m *InvMember) GetByID(c context.Context, id int64) error {
	return app.DB().WithContext(c).First(m, id).Error
}

// Create 创建res_invest_member记录
func (m *InvMember) Create(c context.Context) error {
	return app.DB().WithContext(c).Create(m).Error
}

// Update 更新res_invest_member记录
func (m *InvMember) Update(c context.Context) error {
	return app.DB().WithContext(c).Save(m).Error
}

// Delete 软删除res_invest_member记录
func (m *InvMember) Delete(c context.Context) error {
	return app.DB().WithContext(c).Delete(m).Error
}

// IsEmpty 检查模型是否为空
func (m *InvMember) IsEmpty() bool {
	return m == nil || m.Id == 0
}

// Find 查询res_invest_member列表
func (l *InvMemberList) Find(c context.Context, funcs ...func(*gorm.DB) *gorm.DB) error {
	return app.DB().WithContext(c).Model(&InvMember{}).Scopes(funcs...).Find(l).Error
}

// GetTotal 获取res_invest_member总数
func (l *InvMemberList) GetTotal(c context.Context, query ...func(*gorm.DB) *gorm.DB) (int64, error) {
	var count int64
	err := app.DB().WithContext(c).Model(&InvMember{}).Scopes(query...).Count(&count).Error
	return count, err
}